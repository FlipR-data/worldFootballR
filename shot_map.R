...
mbappe  <- understat_player_shots(player_url = "https://understat.com/player/3423")  %>% filter(season  == 2023,result=="Goal")
rodri   <- understat_player_shots(player_url = "https://understat.com/player/2496")  %>% filter(season  == 2023,result=="Goal")
kane    <- understat_player_shots(player_url = "https://understat.com/player/647")   %>% filter(season  == 2023,result=="Goal")
haaland <- understat_player_shots(player_url = "https://understat.com/player/8260")  %>% filter(season  == 2023,result=="Goal")
...
map_shot <- function(data,player_name) {
   ...
   res_shot<- ggplot(data) +
              ...
              geom_jitter(aes(x = x, y = y,colour = shotType), width = 1, height =  1,size = 1) +
              theme_pitch()                                                                     +
